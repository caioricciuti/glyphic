<script lang="ts">
  ...
  import {
    Search, Sparkles, Bot, Shield, Zap, Server, LayoutGrid,
    Check, Download,
  } from "lucide-svelte";
  ...
  const CATEGORIES = [
    ...
    { id: "skill" as const, label: "Skills", icon: Sparkles, color: "text-warning" },
  ...
  const TEMPLATES: Template[] = [
    ...
    // Rules
    { id: "rule-ts", name: "TypeScript Strict", description: "Enforce strict TypeScript patterns", category: "rule", paths: ["**/*.ts", "**/*.tsx"], content: "# TypeScript Rules\n\n- Strict mode, no `any`\n- Named exports over defaults\n- Explicit return types\n- No unused variables" },
    { id: "rule-api", name: "API Design", description: "REST API conventions", category: "rule", paths: ["src/api/**/*"], content: "# API Rules\n\n- Validate all input\n- Standard error format\n- Proper HTTP status codes\n- Pagination for lists" },
    { id: "rule-testing", name: "Testing", description: "Test standards and patterns", category: "rule", paths: ["**/*.test.*"], content: "# Testing Rules\n\n- Co-locate tests with source\n- Descriptive names\n- No test dependencies\n- 80%+ coverage on critical paths" },
    { id: "rule-security", name: "Security", description: "Security best practices", category: "rule", paths: [], content: "# Security Rules\n\n- Never commit secrets\n- Sanitize user input\n- Parameterized queries\n- HTTPS for external APIs" },
    { id: "rule-git", name: "Git Conventions", description: "Commit and branching standards", category: "rule", paths: [], content: "# Git Rules\n\n- Conventional commits\n- Atomic commits\n- Imperative mood\n- Reference issues" },
    { id: "rule-errors", name: "Error Handling", description: "Error handling patterns", category: "rule", paths: ["src/**/*"], content: "# Error Handling\n\n- Never swallow errors\n- Custom error classes\n- Include context\n- User-friendly messages" },
    { id: "rule-a11y", name: "Accessibility", description: "UI accessibility standards", category: "rule", paths: ["**/*.svelte", "**/*.tsx"], content: "# Accessibility\n\n- Alt text on images\n- Semantic HTML\n- Keyboard navigation\n- Color contrast\n- Aria labels" },
    { id: "rule-perf", name: "Performance", description: "Performance guidelines", category: "rule", paths: [], content: "# Performance\n\n- Avoid N+1 queries\n- Pagination\n- Lazy loading\n- Profile before optimizing" },

    // Hooks
    { id: "hook-bash", name: "Bash Validator", description: "Validate bash commands before execution", category: "hook", event: "PreToolUse", matcher: "Bash", hookType: "command", hookValue: ".claude/hooks/validate.sh" },
    { id: "hook-webhook", name: "HTTP Webhook", description: "Send events to an endpoint", category: "hook", event: "PreToolUse", matcher: "", hookType: "http", hookValue: "http://localhost:8080/hook" },
    { id: "hook-guard", name: "Prompt Guard", description: "AI validates actions", category: "hook", event: "PreToolUse", matcher: "", hookType: "prompt", hookValue: "Check if this action is safe" },
    { id: "hook-log", name: "Log Usage", description: "Log tool usage to file", category: "hook", event: "PostToolUse", matcher: "", hookType: "command", hookValue: "echo \"$(date): tool used\" >> ~/.claude/usage.log" },
    ...
    // MCP
    { id: "mcp-fs", name: "Filesystem", description: "Read/write local files", category: "mcp", mcpType: "stdio", mcpCommand: "npx", mcpArgs: "-y @modelcontextprotocol/server-filesystem /path/to/dir" },
    { id: "mcp-github", name: "GitHub", description: "GitHub API access", category: "mcp", mcpType: "stdio", mcpCommand: "npx", mcpArgs: "-y @modelcontextprotocol/server-github" },
    ...
    { id: "mcp-memory", name: "Memory", description: "Persistent memory store", category: "mcp", mcpType: "stdio", mcpCommand: "npx", mcpArgs: "-y @modelcontextprotocol/server-memory" },
    ...
    try {
      ...
      switch (template.category) {
        ...
        case "agent":
          ...
          break;
        case "rule":
          await api.rules.write("global", name + ".md", template.paths ?? [], template.content ?? "", pp);
          break;
        case "hook": {
          const hooks = await api.hooks.get("global") as Record<string, Array<{ matcher?: string; hooks: HookHandler[] }>>;
          ...
          hooks[event].push({ matcher: template.matcher || undefined, hooks: [handler] });
...
<div class="p-6 overflow-y-auto h-full space-y-4">
  ...
  <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
    {#each filtered as template}
      ...
      <div class="bg-bg-secondary border border-border rounded-lg p-4 flex flex-col justify-between hover:border-accent/30 transition-colors group">
        <div>
          <div class="flex items-center justify-between mb-2">
            <span class="text-[10px] px-2 py-0.5 rounded-full border {CATEGORY_COLORS[template.category]}">{template.category}</span>
          </div>
