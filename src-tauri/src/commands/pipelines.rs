...
fn execute_node(node: &PipelineNode, context: &Option<String>) -> Result<String, String> {
    let ctx = context.as_deref();
    match node.node_type.as_str() {
        "bash" | "github" => {
            let raw = node.config.get("command").and_then(|c| c.as_str()).unwrap_or("echo 'no command'");
            let command = if let Some(c) = ctx {
                raw.replace("{{input}}", c).replace("$INPUT", c)
            } else { raw.to_string() };
        ...
        "http" => {
            ...
            let body = if let Some(c) = ctx {
                node.config.get("body").and_then(|b| b.as_str()).unwrap_or("")
                    .replace("{{input}}", c)
            } else {
                node.config.get("body").and_then(|b| b.as_str()).unwrap_or("").to_string()
            };
...
pub fn start_pipeline_run(pipeline: Pipeline, app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    std::thread::spawn(move || {
        ...
        for node_id in &sorted {
            ...
            match execute_node(node, &last_output) {
                Ok(output) => {
